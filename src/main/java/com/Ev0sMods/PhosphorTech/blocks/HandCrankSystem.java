...
/** ECS ticking system for {@link HandCrankState}. */
public class HandCrankSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, HandCrankState> componentType;
    ...
    public HandCrankSystem(ComponentType<ChunkStore, HandCrankState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        HandCrankState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, HandCrankState> componentType;
        public Cleanup(ComponentType<ChunkStore, HandCrankState> componentType) { this.componentType = componentType; }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            HandCrankState state = h.getComponent(componentType);
            if (state != null) {
                try {
