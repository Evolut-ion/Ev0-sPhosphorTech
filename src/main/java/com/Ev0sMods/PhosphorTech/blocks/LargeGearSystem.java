...
/** ECS ticking system for {@link LargeGearState}. */
public class LargeGearSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, LargeGearState> componentType;
    ...
    public LargeGearSystem(ComponentType<ChunkStore, LargeGearState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        LargeGearState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, LargeGearState> componentType;
        public Cleanup(ComponentType<ChunkStore, LargeGearState> componentType) { this.componentType = componentType; }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            LargeGearState state = h.getComponent(componentType);
            if (state != null) {
                try {
