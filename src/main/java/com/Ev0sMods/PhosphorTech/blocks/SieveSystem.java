...
/** ECS ticking system for {@link SieveState}. */
public class SieveSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, SieveState> componentType;
    ...
    public SieveSystem(ComponentType<ChunkStore, SieveState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        SieveState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, SieveState> componentType;
        public Cleanup(ComponentType<ChunkStore, SieveState> componentType) { this.componentType = componentType; }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            SieveState state = h.getComponent(componentType);
            if (state != null) {
                try {
