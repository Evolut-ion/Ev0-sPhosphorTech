...
/**
 * ECS ticking system for {@link SteamGeneratorState}.
 */
public class SteamGeneratorSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, SteamGeneratorState> componentType;
    ...
    public SteamGeneratorSystem(ComponentType<ChunkStore, SteamGeneratorState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        SteamGeneratorState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, SteamGeneratorState> componentType;
        public Cleanup(ComponentType<ChunkStore, SteamGeneratorState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            SteamGeneratorState state = h.getComponent(componentType);
            if (state != null) {
                try {
