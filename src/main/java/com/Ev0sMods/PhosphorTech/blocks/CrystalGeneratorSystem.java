...
/**
 * ECS ticking system for {@link CrystalGeneratorState}.
 * Registered during plugin setup via {@code csr.registerSystem(...)}.
 */
public class CrystalGeneratorSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, CrystalGeneratorState> componentType;
    ...
    public CrystalGeneratorSystem(ComponentType<ChunkStore, CrystalGeneratorState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        CrystalGeneratorState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, CrystalGeneratorState> componentType;
        public Cleanup(ComponentType<ChunkStore, CrystalGeneratorState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            CrystalGeneratorState state = h.getComponent(componentType);
            if (state != null) {
                try {
