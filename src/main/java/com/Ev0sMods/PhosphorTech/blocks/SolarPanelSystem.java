...
/** Drives {@link SolarPanelState#tick} once per server tick. */
public class SolarPanelSystem extends EntityTickingSystem<ChunkStore> {
    @Nonnull
    private final ComponentType<ChunkStore, SolarPanelState> componentType;
    ...
    public SolarPanelSystem(ComponentType<ChunkStore, SolarPanelState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        SolarPanelState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, SolarPanelState> componentType;
        public Cleanup(ComponentType<ChunkStore, SolarPanelState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            SolarPanelState state = h.getComponent(componentType);
            if (state != null) {
                state.invalidate();
