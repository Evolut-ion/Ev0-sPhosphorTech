...
/** ECS ticking system for {@link ClutchState}. */
public class ClutchSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, ClutchState> componentType;
    ...
    public ClutchSystem(ComponentType<ChunkStore, ClutchState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        ClutchState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, ClutchState> componentType;
        public Cleanup(ComponentType<ChunkStore, ClutchState> componentType) { this.componentType = componentType; }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            ClutchState state = h.getComponent(componentType);
            if (state != null) {
                try {
