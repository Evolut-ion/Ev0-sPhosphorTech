...
/** ECS ticking system for {@link LeafSpringFlywheelCapacitorState}. */
public class LeafSpringFlywheelCapacitorSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, LeafSpringFlywheelCapacitorState> componentType;
    ...
    public LeafSpringFlywheelCapacitorSystem(
            ComponentType<ChunkStore, LeafSpringFlywheelCapacitorState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        LeafSpringFlywheelCapacitorState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, LeafSpringFlywheelCapacitorState> componentType;
        public Cleanup(ComponentType<ChunkStore, LeafSpringFlywheelCapacitorState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            LeafSpringFlywheelCapacitorState state = h.getComponent(componentType);
            if (state != null) {
                try {
