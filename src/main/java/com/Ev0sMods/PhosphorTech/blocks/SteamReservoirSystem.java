...
/** Drives {@link SteamReservoirState#tick} once per server tick. */
public class SteamReservoirSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, SteamReservoirState> componentType;
    ...
    public SteamReservoirSystem(ComponentType<ChunkStore, SteamReservoirState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        SteamReservoirState res = archetypeChunk.getComponent(index, componentType);
        if (res != null) res.tick(dt, index, archetypeChunk, store, commandBuffer);
    }
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, SteamReservoirState> componentType;
        public Cleanup(ComponentType<ChunkStore, SteamReservoirState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            SteamReservoirState state = h.getComponent(componentType);
            if (state != null) {
                try {
