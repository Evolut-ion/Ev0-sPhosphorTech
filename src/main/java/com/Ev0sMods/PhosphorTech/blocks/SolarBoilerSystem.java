...
/** Drives {@link SolarBoilerState#tick} once per server tick. */
public class SolarBoilerSystem extends EntityTickingSystem<ChunkStore> {
    @Nonnull
    private final ComponentType<ChunkStore, SolarBoilerState> componentType;
    ...
    public SolarBoilerSystem(ComponentType<ChunkStore, SolarBoilerState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        SolarBoilerState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, SolarBoilerState> componentType;
        public Cleanup(ComponentType<ChunkStore, SolarBoilerState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            SolarBoilerState state = h.getComponent(componentType);
            if (state != null) {
                try {
