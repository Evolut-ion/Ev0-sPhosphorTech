...
/** ECS ticking system for {@link CokeOvenState}. */
public class CokeOvenSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, CokeOvenState> componentType;
    ...
    public CokeOvenSystem(ComponentType<ChunkStore, CokeOvenState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        CokeOvenState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, CokeOvenState> componentType;
        public Cleanup(ComponentType<ChunkStore, CokeOvenState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            CokeOvenState state = h.getComponent(componentType);
            if (state != null) {
                try {
