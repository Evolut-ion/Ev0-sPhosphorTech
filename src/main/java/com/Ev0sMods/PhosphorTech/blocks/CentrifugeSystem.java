...
/** ECS ticking system for {@link CentrifugeState}. */
public class CentrifugeSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, CentrifugeState> componentType;
    ...
    public CentrifugeSystem(ComponentType<ChunkStore, CentrifugeState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        CentrifugeState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, CentrifugeState> componentType;
        public Cleanup(ComponentType<ChunkStore, CentrifugeState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            CentrifugeState state = h.getComponent(componentType);
            if (state != null) {
                try {
