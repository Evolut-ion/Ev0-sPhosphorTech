...
/** ECS ticking system for {@link CompressorState}. */
public class CompressorSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, CompressorState> componentType;

    public CompressorSystem(ComponentType<ChunkStore, CompressorState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        CompressorState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, CompressorState> componentType;
        public Cleanup(ComponentType<ChunkStore, CompressorState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            CompressorState state = h.getComponent(componentType);
            if (state != null) {
                try {
