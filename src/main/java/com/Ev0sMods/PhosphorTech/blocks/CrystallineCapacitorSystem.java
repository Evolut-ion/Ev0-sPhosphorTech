...
/**
 * Drives {@link CrystallineCapacitorState#tick} once per server tick.
 *
 * <p>Runs after fluid systems so CF produced this tick can be imported
 * into the capacitor before it exports to downstream receivers.
 */
public class CrystallineCapacitorSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, CrystallineCapacitorState> componentType;
    ...
    public CrystallineCapacitorSystem(ComponentType<ChunkStore, CrystallineCapacitorState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        CrystallineCapacitorState cap = archetypeChunk.getComponent(index, componentType);
        if (cap != null) {
            cap.tick(dt, index, archetypeChunk, store, commandBuffer);
        }
    }
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, CrystallineCapacitorState> componentType;
        public Cleanup(ComponentType<ChunkStore, CrystallineCapacitorState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            CrystallineCapacitorState state = h.getComponent(componentType);
            if (state != null) {
                try {
