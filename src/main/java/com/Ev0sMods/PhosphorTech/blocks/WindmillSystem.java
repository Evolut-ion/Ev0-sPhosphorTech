...
/** Drives {@link WindmillState#tick} once per server tick. */
public class WindmillSystem extends EntityTickingSystem<ChunkStore> {
    @Nonnull
    private final ComponentType<ChunkStore, WindmillState> componentType;
    ...
    public WindmillSystem(ComponentType<ChunkStore, WindmillState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        WindmillState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, WindmillState> componentType;
        public Cleanup(ComponentType<ChunkStore, WindmillState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            WindmillState state = h.getComponent(componentType);
            if (state != null) {
                state.invalidate();
