...
/** ECS ticking system for {@link AlloySmelterState}. */
public class AlloySmelterSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, AlloySmelterState> componentType;
    ...
    public AlloySmelterSystem(ComponentType<ChunkStore, AlloySmelterState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        AlloySmelterState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, AlloySmelterState> componentType;
        public Cleanup(ComponentType<ChunkStore, AlloySmelterState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            AlloySmelterState state = h.getComponent(componentType);
            if (state != null) {
                try {
                ...
                state.removed = true;
                com.Ev0sMods.PhosphorTech.energy.CrystallineFluxNetwork
                        .unregisterExact(state.getPosition(), state);
            }
        }
