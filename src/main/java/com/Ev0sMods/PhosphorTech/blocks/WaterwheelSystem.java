...
/** ECS ticking system for {@link WaterwheelState}. */
public class WaterwheelSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, WaterwheelState> componentType;
    ...
    public WaterwheelSystem(ComponentType<ChunkStore, WaterwheelState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        WaterwheelState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, WaterwheelState> componentType;
        public Cleanup(ComponentType<ChunkStore, WaterwheelState> componentType) { this.componentType = componentType; }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            WaterwheelState state = h.getComponent(componentType);
            if (state != null) {
                try {
