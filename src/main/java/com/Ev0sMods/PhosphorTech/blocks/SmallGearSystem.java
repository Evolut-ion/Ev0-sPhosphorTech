...
/** ECS ticking system for {@link SmallGearState}. */
public class SmallGearSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull private final ComponentType<ChunkStore, SmallGearState> componentType;
    ...
    public SmallGearSystem(ComponentType<ChunkStore, SmallGearState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        SmallGearState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, SmallGearState> componentType;
        public Cleanup(ComponentType<ChunkStore, SmallGearState> componentType) { this.componentType = componentType; }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            SmallGearState state = h.getComponent(componentType);
            if (state != null) {
