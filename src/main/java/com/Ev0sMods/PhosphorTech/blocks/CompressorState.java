...
import com.Ev0sMods.PhosphorTech.ui.CompressorUI;
...
/**
 * ECS component for the Compressor block (electrical plate-making machine).
 *
 * <p>CF-powered machine that converts ingots and fired bricks into flat plates.
 * Shares {@link ProcessingRecipeRegistry#PRESS} recipes with the hand-operated Press.
 * Slot 0 = input, slot 1 = output. F-key interaction ({@code OpenCompressor}).
 */
...
public class CompressorState
    ...
    public static ComponentType<ChunkStore, CompressorState> COMPONENT_TYPE;

    public static final ConcurrentHashMap<String, CompressorState> REGISTRY = new ConcurrentHashMap<>();

    public static @Nullable CompressorState getAt(Vector3i pos) {
    ...
    public static final BuilderCodec<CompressorState> CODEC =
            BuilderCodec.builder(CompressorState.class, CompressorState::new)
    ...
    private CompressorState(CompressorState other) {
        this();
    ...
    @Override public CompressorState clone()    { return new CompressorState(this); }
    ...
    public void tick(float dt, int index,
        ...
        uiTick++;
        if (CompressorUI.hasWatcher(cachedPosition) && uiTick >= 20) {
            uiTick = 0;
            CompressorUI.tickRefresh(this, store, cachedPosition);
        }
    ...
    private static Map<?, ?> entityRefsViaReflection(BlockComponentChunk bcc) {
        try {
            if (!entityRefsMethodResolved) {
                synchronized (CompressorState.class) {
                    if (!entityRefsMethodResolved) {
