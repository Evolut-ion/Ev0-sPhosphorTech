...
/** ECS ticking system for {@link CrusherState}. */
public class CrusherSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, CrusherState> componentType;
    ...
    public CrusherSystem(ComponentType<ChunkStore, CrusherState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        CrusherState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, CrusherState> componentType;
        public Cleanup(ComponentType<ChunkStore, CrusherState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            CrusherState state = h.getComponent(componentType);
            if (state != null) {
                try {
