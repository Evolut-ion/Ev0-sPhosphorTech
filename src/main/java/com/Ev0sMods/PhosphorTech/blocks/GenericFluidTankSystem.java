...
/** Drives {@link GenericFluidTankState#tick} once per server tick. */
public class GenericFluidTankSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, GenericFluidTankState> componentType;
    ...
    public GenericFluidTankSystem(ComponentType<ChunkStore, GenericFluidTankState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        GenericFluidTankState tank = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        ...
        public Cleanup(ComponentType<ChunkStore, GenericFluidTankState> componentType) {
            this.componentType = componentType;
        }

        @Override
        public void onEntityAdd(@Nonnull Holder<ChunkStore> h,
                                @Nonnull AddReason r,
                                @Nonnull Store<ChunkStore> s) {}

        @Override
        public void onEntityRemoved(@Nonnull Holder<ChunkStore> h,
                                    @Nonnull RemoveReason r,
                                    @Nonnull Store<ChunkStore> s) {
            GenericFluidTankState state = h.getComponent(componentType);
            if (state != null) {
                try {
