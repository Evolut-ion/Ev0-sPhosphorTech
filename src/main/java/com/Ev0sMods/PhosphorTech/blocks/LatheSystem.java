...
/** ECS ticking system for {@link LatheState}. */
public class LatheSystem extends EntityTickingSystem<ChunkStore> {

    @Nonnull
    private final ComponentType<ChunkStore, LatheState> componentType;

    public LatheSystem(ComponentType<ChunkStore, LatheState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        LatheState state = archetypeChunk.getComponent(index, componentType);
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, LatheState> componentType;
        public Cleanup(ComponentType<ChunkStore, LatheState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            LatheState state = h.getComponent(componentType);
            if (state != null) {
                try {
