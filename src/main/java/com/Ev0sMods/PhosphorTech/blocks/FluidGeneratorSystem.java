...
/** Drives {@link FluidGeneratorState#tick} once per server tick. */
public class FluidGeneratorSystem extends EntityTickingSystem<ChunkStore> {
    @Nonnull
    private final ComponentType<ChunkStore, FluidGeneratorState> componentType;
    ...
    public FluidGeneratorSystem(ComponentType<ChunkStore, FluidGeneratorState> componentType) {
    ...
    public void tick(float dt, int index,
        ...
        FluidGeneratorState state = archetypeChunk.getComponent(index, componentType);
        if (state != null) {
    ...
    public static class Cleanup extends HolderSystem<ChunkStore> {
        private final ComponentType<ChunkStore, FluidGeneratorState> componentType;
        public Cleanup(ComponentType<ChunkStore, FluidGeneratorState> componentType) {
            this.componentType = componentType;
        }
        ...
        @Override public void onEntityRemoved(@Nonnull Holder<ChunkStore> h, @Nonnull RemoveReason r, @Nonnull Store<ChunkStore> s) {
            FluidGeneratorState state = h.getComponent(componentType);
            if (state != null) {
                state.invalidate();
