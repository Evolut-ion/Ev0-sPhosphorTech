...
/** HyUI page for the Alloy Smelter block. */
@SuppressWarnings({"removal", "unchecked"})
public final class AlloySmelterUIPage {

    private AlloySmelterUIPage() {}
    ...
    public static void ping(Vector3i pos) {
        SESSIONS.forEach((pr, s) -> {
            if (!posKey(s.blockPos()).equals(posKey(pos)) || s.page() == null) return;
            s.page().updatePage(false);
        });
    }
    ...
    private static void renderPage(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            AlloySmelterState state = lookupState(store, pos);
            ...
            builder.addEventListener("as-close-btn", CustomUIEventBindingType.Activating,
                    (ign, ctx) -> {
            ...
            for (SlotInfo info : slots) {
                ...
                if ("as-slot-2-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> transferOutputToPlayer(playerRef, entityRef, store, pos, (short) 2));
                } else if ("as-slot-3-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> transferOutputToPlayer(playerRef, entityRef, store, pos, (short) 3));
                } else if ("as-slot-0-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> takeFromBlock(playerRef, entityRef, store, pos, (short) 0));
                } else if ("as-slot-1-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> takeFromBlock(playerRef, entityRef, store, pos, (short) 1));
                } else {
                    // player inventory slot → transfer item into input slot 0 first, then 1
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> transferItemFromPlayer(playerRef, entityRef, store, pos, src, slot));
                }
            }
            ...
            HytaleLogger.getLogger().atWarning().log("[AlloySmelterUI] renderPage failed: " + t);
        }
    }

    private static void partialRefresh(HyUIPage page, AlloySmelterState state) {
        if (page == null || state == null) return;

        int cfFill = (int)(288.0 * state.getCFStored() / AlloySmelterState.CF_CAPACITY);
        page.getById("as-cf-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(cfFill));
        page.getById("as-cf-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(state.getCFStored() + " / " + state.getCFCapacity() + " CF"));

        int progFill = state.processing
                ? (int)(288.0 * state.processTimer / Math.max(1, state.ticksNeeded)) : 0;
        page.getById("as-prog-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(progFill));
        page.getById("as-prog-text", LabelBuilder.class)
                .ifPresent(b -> b.withText(buildProgText(state.processing,
                        state.processTimer, state.ticksNeeded)));

        String statusText  = state.processing ? "Smelting..." : "Idle";
        String statusColor = state.processing ? "#a5d6a7" : "#ef9a9a";
        page.getById("as-status-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(statusText).withStyle(new HyUIStyle().setTextColor(statusColor)));

        updateSlotIcon(page, state.getItemContainer(), (short) 0, "as-slot-0");
        updateSlotIcon(page, state.getItemContainer(), (short) 1, "as-slot-1");
        updateSlotIcon(page, state.getItemContainer(), (short) 2, "as-slot-2");
        updateSlotIcon(page, state.getItemContainer(), (short) 3, "as-slot-3");
    }
    ...
    private static void updateSlotIcon(HyUIPage page, ItemContainer ic, short slot, String slotId) {
        ...
        page.getById(slotId + "-name", LabelBuilder.class).ifPresent(b -> b.withText(name));
        page.getById(slotId + "-qty",  LabelBuilder.class).ifPresent(b -> b.withText(qty));
    }

    // ── HTML builder ──────────────────────────────────────────────────────────

    private static String buildHtml(AlloySmelterState state, Inventory inv, List<SlotInfo> slotsOut) {
        boolean processing = state != null && state.processing;
        String statusColor = processing ? "#a5d6a7" : "#ef9a9a";
        String statusText  = processing ? "Smelting..." : "Idle";
        long cfStored  = state != null ? state.getCFStored() : 0L;
        int  cfFill    = state != null ? (int)(288.0 * cfStored / AlloySmelterState.CF_CAPACITY) : 0;
        String cfText  = cfStored + " / " + AlloySmelterState.CF_CAPACITY + " CF";
        String progText = buildProgText(processing,
                state != null ? state.processTimer : 0, state != null ? state.ticksNeeded : 120);

        // Collect slot item data for initial HTML render
        String[] slotItemId = new String[4];
        int[]    slotQty    = new int[4];
        if (state != null) {
            ItemContainer ic = state.getItemContainer();
            if (ic != null) {
                for (short i = 0; i < 4; i++) {
                    ItemStack s = ic.getItemStack(i);
                    if (s != null && !s.isEmpty()) { slotItemId[i] = s.getItemId(); slotQty[i] = s.getQuantity(); }
                }
                slotsOut.add(new SlotInfo("as-slot-0-btn", ic, (short) 0));
                slotsOut.add(new SlotInfo("as-slot-1-btn", ic, (short) 1));
                slotsOut.add(new SlotInfo("as-slot-2-btn", ic, (short) 2));
                slotsOut.add(new SlotInfo("as-slot-3-btn", ic, (short) 3));
            }
        }
        ...
        String leftPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 330; padding-top: 8; padding-bottom: 8;
                    ...
                    <p class="title-label">&#9830; Alloy Smelter</p>
                    <div class="separator"></div>
                    <p class="section-label">Status</p>
                    <p id="as-status-val" class="info-label" style="color: %s;">%s</p>
                    <div class="separator"></div>
                    ...
                    <div style="layout-mode: Left; anchor-width: 288; anchor-height: 18;
                        ...
                        <div id="as-cf-fill" style="anchor-width: %d; anchor-height: 18;
                             background-color: #7c4dff; border-radius: 9;"></div>
                    </div>
                    <p id="as-cf-val" class="info-label">%s</p>
                    <div class="separator"></div>
                    <p class="section-label">Progress</p>
                    <p id="as-prog-text" class="info-label" style="margin-top: 4; margin-bottom: 4;">%s</p>
                    <p class="hint-label">Combines two materials into alloys. Requires CF energy.</p>
                    <div style="layout-mode: Top; horizontal-align: center; padding-top: 8;">
                        <button id="as-close-btn" class="secondary-button"
        ...
        String rightPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 210; padding-top: 8; padding-bottom: 8;
                    ...
                    %s
                    %s
                    <p class="hint-label" style="horizontal-align: center;">&#9660; Smelts Into &#9660;</p>
                ...
                """,
                buildInputSlotHtml(slotItemId[0], slotQty[0], "Input 1", "as-slot-0"),
                buildInputSlotHtml(slotItemId[1], slotQty[1], "Input 2", "as-slot-1"),
                buildOutputSlotHtml(slotItemId[2], slotQty[2], "Output 1", "as-slot-2"),
                buildOutputSlotHtml(slotItemId[3], slotQty[3], "Output 2", "as-slot-3"));
        ...
        return STYLE + String.format("""
                <div style="anchor-width: 100%%; anchor-height: 100%%;
                    ...
                    <div class="decorated-container" data-hyui-title="Alloy Smelter"
    ...
    /** Transfer a player inventory item into whichever input slot (0 then 1) has space. */
    private static void transferItemFromPlayer(PlayerRef playerRef, Ref<EntityStore> entityRef,
                                                Store<EntityStore> store, Vector3i pos,
                                                ItemContainer srcContainer, short srcSlot) {
        try {
            ...
            // Find the first input slot that is empty or has the same item with room
            for (short target = 0; target <= 1; target++) {
                ItemStack cur = blockIc.getItemStack(target);
                if (cur == null || cur.isEmpty()) {
                    blockIc.setItemStackForSlot(target, moving);
                    srcContainer.setItemStackForSlot(srcSlot, ItemStack.EMPTY);
                    state.uiDirty = true;
                    refreshAfterItemMove(playerRef, store, entityRef, state, blockIc);
                    return;
                }
                if (cur.getItemId().equals(moving.getItemId()) && cur.getQuantity() < 99) {
                    int add = Math.min(moving.getQuantity(), 99 - cur.getQuantity());
                    ItemStack merged = new ItemStack(cur.getItemId(), cur.getQuantity() + add, null);
                    blockIc.setItemStackForSlot(target, merged);
                    int rem = moving.getQuantity() - add;
                    srcContainer.setItemStackForSlot(srcSlot,
                            rem > 0 ? new ItemStack(moving.getItemId(), rem, null) : ItemStack.EMPTY);
                    state.uiDirty = true;
                    refreshAfterItemMove(playerRef, store, entityRef, state, blockIc);
                    return;
                }
    ...
    private static void takeFromBlock(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            state.uiDirty = true;
            if (ps.page() != null) {
                updateSlotIcon(ps.page(), blockIc, blockSlot, "as-slot-" + blockSlot);
                refreshAllInvSlots(ps.page(), inv);
                ps.page().updatePage(false);
            }
        } catch (Throwable ignored) {}
    }
    ...
    private static void refreshAfterItemMove(PlayerRef playerRef, Store<EntityStore> store,
                                              Ref<EntityStore> entityRef,
                                              AlloySmelterState state, ItemContainer blockIc) {
        PlayerSession ps = SESSIONS.get(playerRef);
        if (ps == null || ps.page() == null) return;
        HyUIPage page = ps.page();
        for (short i = 0; i < 4; i++) updateSlotIcon(page, blockIc, i, "as-slot-" + i);
        try {
            Player player = store.getComponent(ps.entityRef(), Player.getComponentType());
    ...
    private static AlloySmelterState lookupState(Store<EntityStore> store, Vector3i pos) {
        Object node = CrystallineFluxNetwork.getAt(pos);
        if (node instanceof AlloySmelterState s) return s;
        return null;
    }
