...
/**
 * Reflection-based bridge to {@link MechanicalGrinderUIPage}.
 * Safe no-op when HyUI is absent at runtime.
 */
public final class MechanicalGrinderUI {

    private MechanicalGrinderUI() {}
    ...
    private static synchronized void ensureInit() {
        ...
        if (HYUI_PRESENT) {
            try {
                Class<?> page = Class.forName("com.Ev0sMods.PhosphorTech.ui.MechanicalGrinderUIPage", true, Thread.currentThread().getContextClassLoader());
                ...
                mTickRefresh = page.getMethod("tickRefresh",
                        MechanicalGrinderState.class, Store.class, Vector3i.class);
                mPing        = page.getMethod("ping", Vector3i.class);
            } catch (Exception e) {
                HytaleLogger.getLogger().atWarning().log(
                        "[MechanicalGrinderUI] Reflection init failed: " + e.getMessage());
            }
        } else {
            HytaleLogger.getLogger().atInfo().log("[MechanicalGrinderUI] HyUI not present; UI is disabled.");
        }
    }
    ...
    public static void openForced(PlayerRef playerRef, Ref<?> entityRef,
        ...
        try { mOpenForced.invoke(null, playerRef, entityRef, store, pos); }
            ...
            HytaleLogger.getLogger().atWarning().log("[MechanicalGrinderUI] openForced failed: " + t.getMessage());
        }
    }
    ...
    public static void tickRefresh(MechanicalGrinderState state, Store<?> store, Vector3i pos) {
        ...
        try { mTickRefresh.invoke(null, state, store, pos); }
            ...
            HytaleLogger.getLogger().atWarning().log("[MechanicalGrinderUI] tickRefresh failed: " + t.getMessage());
        }
    }
