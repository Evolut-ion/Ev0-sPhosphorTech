...
/** Reflection-based bridge to {@link CentrifugeUIPage}. Safe no-op when HyUI is absent. */
public final class CentrifugeUI {

    private CentrifugeUI() {}
    ...
    private static synchronized void ensureInit() {
        ...
        if (HYUI_PRESENT) {
            try {
                Class<?> page = Class.forName("com.Ev0sMods.PhosphorTech.ui.CentrifugeUIPage", true, Thread.currentThread().getContextClassLoader());
                ...
                mTickRefresh = page.getMethod("tickRefresh",
                        CentrifugeState.class, Store.class, Vector3i.class);
                mPing        = page.getMethod("ping", Vector3i.class);
            } catch (Exception e) {
                HytaleLogger.getLogger().atWarning().log(
                        "[CentrifugeUI] Reflection init failed: " + e.getMessage());
            }
        } else {
            HytaleLogger.getLogger().atInfo().log("[CentrifugeUI] HyUI not present; UI is disabled.");
        }
    }
    ...
    public static void openForced(PlayerRef playerRef, Ref<?> entityRef,
        ...
        try { mOpenForced.invoke(null, playerRef, entityRef, store, pos); }
            ...
            HytaleLogger.getLogger().atWarning().log("[CentrifugeUI] openForced failed: " + t.getMessage());
        }
    }
    ...
    public static void tickRefresh(CentrifugeState state, Store<?> store, Vector3i pos) {
        ...
        try { mTickRefresh.invoke(null, state, store, pos); }
            ...
            HytaleLogger.getLogger().atWarning().log("[CentrifugeUI] tickRefresh failed: " + t.getMessage());
        }
    }
