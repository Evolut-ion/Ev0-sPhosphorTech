...
/**
 * Reflection-based bridge to {@link SteamReservoirUIPage}.
 *
 * <p>All code that needs to open or update the Steam Reservoir UI goes
 * through this class so that HyUI remains an optional compile-time dependency.
 */
public final class SteamReservoirUI {

    private SteamReservoirUI() {}
        ...
        if (HYUI_PRESENT) {
            try {
                Class<?> page = Class.forName(
                        "com.Ev0sMods.PhosphorTech.ui.SteamReservoirUIPage");
                ...
                mTickRefresh = page.getMethod("tickRefresh",
                        SteamReservoirState.class, Store.class, Vector3i.class);
            } catch (Exception e) {
                HytaleLogger.getLogger().atWarning().log(
                        "[SteamReservoirUI] Reflection init failed: " + e.getMessage());
            }
        } else {
            HytaleLogger.getLogger().atInfo().log(
                    "[SteamReservoirUI] HyUI not present; UI is disabled.");
        }
    }
    ...
    public static void openForced(PlayerRef playerRef, Ref<?> entityRef,
        ...
        try {
            ...
            HytaleLogger.getLogger().atWarning().log(
                    "[SteamReservoirUI] openForced failed: " + t.getMessage());
        }
    }
    ...
    public static boolean hasWatcher(Vector3i pos) {
        ...
        try {
            ...
            HytaleLogger.getLogger().atWarning().log(
                    "[SteamReservoirUI] hasWatcher failed: " + t.getMessage());
            return false;
        }
    }

    @SuppressWarnings("unchecked")
    public static void tickRefresh(SteamReservoirState state, Store<?> store, Vector3i pos) {
        if (!isAvailable()) return;
        try {
            mTickRefresh.invoke(null, state, store, pos);
        } catch (Throwable t) {
            HytaleLogger.getLogger().atWarning().log(
                    "[SteamReservoirUI] tickRefresh failed: " + t.getMessage());
        }
    }
}
