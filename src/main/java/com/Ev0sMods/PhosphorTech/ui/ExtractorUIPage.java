...
/** HyUI page for the Extractor block. */
@SuppressWarnings({"removal", "unchecked"})
public final class ExtractorUIPage {

    private ExtractorUIPage() {}
    ...
    private static void renderPage(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ExtractorState state = lookupState(store, pos);
            ...
            builder.addEventListener("ex-close-btn", CustomUIEventBindingType.Activating,
                    (ign, ctx) -> {
            ...
            for (SlotInfo info : slots) {
                ...
                if ("extractor-slot-1-btn".equals(sid)) {
                ...
                } else if ("extractor-slot-0-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
            ...
            HytaleLogger.getLogger().atWarning().log("[ExtractorUI] renderPage failed: " + t);
        }
    }

    private static void partialRefresh(HyUIPage page, ExtractorState state) {
        if (page == null || state == null) return;

        int cfFill = (int)(288.0 * state.getCFStored() / ExtractorState.CF_CAPACITY);
        page.getById("ex-cf-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(cfFill));
        page.getById("ex-cf-val", LabelBuilder.class)
        ...
        page.getById("ex-prog-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(progFill));

        String progText = buildProgText(state.processing, state.processTimer, state.ticksNeeded);
        page.getById("ex-prog-text", LabelBuilder.class).ifPresent(b -> b.withText(progText));

        String statusColor = state.processing ? "#a5d6a7" : "#ef9a9a";
        String statusText  = state.processing ? "Extracting..." : "Idle";
        page.getById("ex-status-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(statusText).withStyle(new HyUIStyle().setTextColor(statusColor)));

        updateSlotIcon(page, state.getItemContainer(), (short) 0, "extractor-slot-0");
        updateSlotIcon(page, state.getItemContainer(), (short) 1, "extractor-slot-1");
    }
    ...
    private static String buildHtml(ExtractorState state, Inventory inv, List<SlotInfo> slotsOut) {
        boolean processing = state != null && state.processing;
        String statusColor = processing ? "#a5d6a7" : "#ef9a9a";
        String statusText  = processing ? "Extracting..." : "Idle";
        int cfFill   = state != null ? (int)(288.0 * state.getCFStored() / ExtractorState.CF_CAPACITY) : 0;
        ...
        if (state != null) {
            ...
            if (ic != null) {
                ...
                slotsOut.add(new SlotInfo("extractor-slot-0-btn", ic, (short) 0));
                slotsOut.add(new SlotInfo("extractor-slot-1-btn", ic, (short) 1));
            }
        }
        ...
        String leftPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 330; padding-top: 8; padding-bottom: 8;
                    ...
                    <p class="title-label">&#9881; Extractor</p>
                    <div class="separator"></div>
                    <p class="section-label">Status</p>
                    <p id="ex-status-val" class="info-label" style="color: %s;">%s</p>
                    <div class="separator"></div>
                    ...
                    <div style="layout-mode: Left; anchor-width: 288; anchor-height: 18;
                        ...
                        <div id="ex-cf-fill" style="anchor-width: %d; anchor-height: 18;
                             background-color: #7c4dff; border-radius: 9;"></div>
                    </div>
                    <p id="ex-cf-val" class="info-label">%s</p>
                    <div class="separator"></div>
                    <p class="section-label">Progress</p>
                    <p id="ex-prog-text" class="info-label" style="margin-top: 4; margin-bottom: 4;">%s</p>
                    <p class="hint-label">Extracts ingredients from organic materials. Requires CF energy.</p>
                    <div style="layout-mode: Top; horizontal-align: center; padding-top: 8;">
                        <button id="ex-close-btn" class="secondary-button"
        ...
        String rightPanel = String.format("""
                ...
                """, buildSlotHtml(slot0Id, slot0Qty, "Input",  "extractor-slot-0"),
                     buildOutputSlotHtml(slot1Id, slot1Qty, "Output", "extractor-slot-1"));
        ...
        return STYLE + String.format("""
                <div style="anchor-width: 100%%; anchor-height: 100%%;
                    ...
                    <div class="decorated-container" data-hyui-title="Extractor"
    ...
    private static void transferItem(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (s != null && s.page() != null) {
                ...
                updateSlotIcon(page, blockIc, targetSlot, "extractor-slot-" + targetSlot);
                try {
    ...
    private static void takeFromBlock(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (ps.page() != null) {
                ...
                updateSlotIcon(page, blockIc, blockSlot, "extractor-slot-" + blockSlot);
    ...
    private static ExtractorState lookupState(Store<EntityStore> store, Vector3i pos) {
        Object node = CrystallineFluxNetwork.getAt(pos);
        if (node instanceof ExtractorState s) return s;
        return null;
    }
    ...
    private static final String STYLE = """
            <style>
                .title-label { font-weight: bold; color: #80cbc4; font-size: 18; padding-top: 8; padding-bottom: 6; }
