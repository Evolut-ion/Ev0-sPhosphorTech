...
/** HyUI page for the Crusher block. */
@SuppressWarnings({"removal", "unchecked"})
public final class CrusherUIPage {

    private CrusherUIPage() {}
    ...
    public static void tickRefresh(CrusherState state, Store<?> store, Vector3i pos) {
        SESSIONS.forEach((playerRef, session) -> {
            if (!posKey(session.blockPos()).equals(posKey(pos))) return;
            HyUIPage page = session.page();
            if (page == null) return;
    ...
    private static void renderPage(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            CrusherState state = lookupState(store, pos);
            ...
            builder.addEventListener("cr-close-btn", CustomUIEventBindingType.Activating,
                    (ign, ctx) -> {
            ...
            for (SlotInfo info : slots) {
                ...
                if ("crusher-slot-1-btn".equals(sid)) {
                ...
                } else if ("crusher-slot-2-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> transferOutputToPlayer(playerRef, entityRef, store, pos, (short) 2));
                } else if ("crusher-slot-0-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
            ...
            HytaleLogger.getLogger().atWarning().log("[CrusherUI] renderPage failed: " + t);
        }
    }

    private static void partialRefresh(HyUIPage page, CrusherState state) {
        if (page == null || state == null) return;

        int cfFill = (int)(288.0 * state.getCFStored() / CrusherState.CF_CAPACITY);
        page.getById("cr-cf-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(cfFill));
        page.getById("cr-cf-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(state.getCFStored() + " / " + state.getCFCapacity() + " CF"));

        int progFill = state.processing ? (int)(288.0 * state.processTimer / 90.0) : 0;
        page.getById("cr-prog-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(progFill));

        String progText = buildProgText(state.processing, state.processTimer, state.ticksNeeded);
        page.getById("cr-prog-text", LabelBuilder.class).ifPresent(b -> b.withText(progText));

        String statusColor = state.processing ? "#a5d6a7" : "#ef9a9a";
        String statusText  = state.processing ? "Processing..." : "Idle";
        page.getById("cr-status-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(statusText).withStyle(new HyUIStyle().setTextColor(statusColor)));

        updateSlotIcon(page, state.getItemContainer(), (short) 0, "crusher-slot-0");
        updateSlotIcon(page, state.getItemContainer(), (short) 1, "crusher-slot-1");
        updateSlotIcon(page, state.getItemContainer(), (short) 2, "crusher-slot-2");
    }
    ...
    private static String buildHtml(CrusherState state, Inventory inv, List<SlotInfo> slotsOut) {
        ...
        int cfFill   = state != null ? (int)(288.0 * state.getCFStored() / CrusherState.CF_CAPACITY) : 0;
        String progText = buildProgText(processing, state != null ? state.processTimer : 0, state != null ? state.ticksNeeded : 90);
        ...
        if (state != null) {
            ...
            if (ic != null) {
                ...
                slotsOut.add(new SlotInfo("crusher-slot-0-btn", ic, (short) 0));
                slotsOut.add(new SlotInfo("crusher-slot-1-btn", ic, (short) 1));
                slotsOut.add(new SlotInfo("crusher-slot-2-btn", ic, (short) 2));
            }
        }
        ...
        String leftPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 330; padding-top: 8; padding-bottom: 8;
                    ...
                    <p class="title-label">&#9874; Crusher</p>
                    <div class="separator"></div>
                    <p class="section-label">Status</p>
                    <p id="cr-status-val" class="info-label" style="color: %s;">%s</p>
                    <div class="separator"></div>
                    ...
                    <div style="layout-mode: Left; anchor-width: 288; anchor-height: 18;
                        ...
                        <div id="cr-cf-fill" style="anchor-width: %d; anchor-height: 18;
                             background-color: #7c4dff; border-radius: 9;"></div>
                    </div>
                    <p id="cr-cf-val" class="info-label">%s</p>
                    <div class="separator"></div>
                    <p class="section-label">Progress</p>
                    <p id="cr-prog-text" class="info-label" style="margin-top: 4; margin-bottom: 4;">%s</p>
                    <p class="hint-label">Crushes ores and minerals into dust. Requires CF energy.</p>
                    <div style="layout-mode: Top; horizontal-align: center; padding-top: 8;">
                        <button id="cr-close-btn" class="secondary-button"
        ...
        String rightPanel = String.format("""
                ...
                """, buildSlotHtml(slot0Id, slot0Qty, "Input",  "crusher-slot-0"),
                     buildOutputSlotHtml(slot1Id, slot1Qty, "Output", "crusher-slot-1"),
                     buildOutputSlotHtml(slot2Id, slot2Qty, "Bonus Output", "crusher-slot-2"));
        ...
        return STYLE + String.format("""
                <div style="anchor-width: 100%%; anchor-height: 100%%;
                    ...
                    <div class="decorated-container" data-hyui-title="Crusher"
    ...
    private static void transferItem(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (s != null && s.page() != null) {
                ...
                updateSlotIcon(page, blockIc, targetSlot, "crusher-slot-" + targetSlot);
                try {
    ...
    private static void takeFromBlock(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (ps.page() != null) {
                ...
                updateSlotIcon(page, blockIc, blockSlot, "crusher-slot-" + blockSlot);
    ...
    private static void updateInvSlotIcon(HyUIPage page, ItemContainer container, short slot) {
        page.getById("inv_h_" + slot + "-icon", ItemIconBuilder.class).ifPresent(b -> b.withItemId(""));
        page.getById("inv_s_" + slot + "-icon", ItemIconBuilder.class).ifPresent(b -> b.withItemId(""));
    }
    ...
    private static void transferOutputToPlayer(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            remaining = mergeIntoExisting(storage, 36, itemId, remaining, MAX);
            // 2. Place into empty slots
            remaining = placeInEmpty(hotbar,  9,  itemId, remaining, MAX);
            remaining = placeInEmpty(storage, 36, itemId, remaining, MAX);
            // Nothing moved — do nothing
            if (remaining == output.getQuantity()) return;
            // Update the machine output slot with whatever remains
    ...
    private static CrusherState lookupState(Store<EntityStore> store, Vector3i pos) {
        Object node = CrystallineFluxNetwork.getAt(pos);
        if (node instanceof CrusherState s) return s;
        return null;
    }
