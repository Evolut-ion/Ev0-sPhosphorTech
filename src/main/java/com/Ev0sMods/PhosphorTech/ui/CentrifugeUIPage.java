...
/** HyUI page for the Centrifuge block. */
@SuppressWarnings({"removal", "unchecked"})
public final class CentrifugeUIPage {

    private CentrifugeUIPage() {}
    ...
    public static void tickRefresh(CentrifugeState state, Store<?> store, Vector3i pos) {
        SESSIONS.forEach((playerRef, session) -> {
            if (!posKey(session.blockPos()).equals(posKey(pos))) return;
            HyUIPage page = session.page();
            if (page == null) return;
    ...
    private static void renderPage(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            CentrifugeState state = lookupState(store, pos);
            ...
            builder.addEventListener("cf-close-btn", CustomUIEventBindingType.Activating,
                    (ign, ctx) -> {
            ...
            for (SlotInfo info : slots) {
                ...
                if ("centrifuge-slot-1-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
                            (ign, ctx) -> transferOutputToPlayer(playerRef, entityRef, store, pos));
                } else if ("centrifuge-slot-0-btn".equals(sid)) {
                    builder.addEventListener(sid, CustomUIEventBindingType.Activating,
            ...
            HytaleLogger.getLogger().atWarning().log("[CentrifugeUI] renderPage failed: " + t);
        }
    }

    private static void partialRefresh(HyUIPage page, CentrifugeState state) {
        if (page == null || state == null) return;

        int cfFill = (int)(288.0 * state.getCFStored() / CentrifugeState.CF_CAPACITY);
        page.getById("cf-cf-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(cfFill));
        page.getById("cf-cf-val", LabelBuilder.class)
        ...
        page.getById("cf-prog-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(progFill));

        String progText = buildProgText(state.processing, state.processTimer, state.ticksNeeded);
        page.getById("cf-prog-text", LabelBuilder.class).ifPresent(b -> b.withText(progText));

        String statusColor = state.processing ? "#a5d6a7" : "#ef9a9a";
        String statusText  = state.processing ? "Centrifuging..." : "Idle";
        page.getById("cf-status-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(statusText).withStyle(new HyUIStyle().setTextColor(statusColor)));

        updateSlotIcon(page, state.getItemContainer(), (short) 0, "centrifuge-slot-0");
        updateSlotIcon(page, state.getItemContainer(), (short) 1, "centrifuge-slot-1");
    }
    ...
    private static String buildHtml(CentrifugeState state, Inventory inv, List<SlotInfo> slotsOut) {
        boolean processing = state != null && state.processing;
        String statusColor = processing ? "#a5d6a7" : "#ef9a9a";
        String statusText  = processing ? "Centrifuging..." : "Idle";
        int cfFill   = state != null ? (int)(288.0 * state.getCFStored() / CentrifugeState.CF_CAPACITY) : 0;
        ...
        if (state != null) {
            ...
            if (ic != null) {
                ...
                slotsOut.add(new SlotInfo("centrifuge-slot-0-btn", ic, (short) 0));
                slotsOut.add(new SlotInfo("centrifuge-slot-1-btn", ic, (short) 1));
            }
        }
        ...
        String leftPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 330; padding-top: 8; padding-bottom: 8;
                    ...
                    <p class="title-label">&#9854; Centrifuge</p>
                    <div class="separator"></div>
                    <p class="section-label">Status</p>
                    <p id="cf-status-val" class="info-label" style="color: %s;">%s</p>
                    <div class="separator"></div>
                    ...
                    <div style="layout-mode: Left; anchor-width: 288; anchor-height: 18;
                        ...
                        <div id="cf-cf-fill" style="anchor-width: %d; anchor-height: 18;
                             background-color: #7c4dff; border-radius: 9;"></div>
                    </div>
                    <p id="cf-cf-val" class="info-label">%s</p>
                    <div class="separator"></div>
                    <p class="section-label">Progress</p>
                    <p id="cf-prog-text" class="info-label" style="margin-top: 4; margin-bottom: 4;">%s</p>
                    <p class="hint-label">Separates fluids and compounds via rapid rotation. Requires CF energy.</p>
                    <div style="layout-mode: Top; horizontal-align: center; padding-top: 8;">
                        <button id="cf-close-btn" class="secondary-button"
        ...
        String rightPanel = String.format("""
                ...
                """, buildSlotHtml(slot0Id, slot0Qty, "Input",  "centrifuge-slot-0"),
                     buildOutputSlotHtml(slot1Id, slot1Qty, "Output", "centrifuge-slot-1"));
        ...
        return STYLE + String.format("""
                <div style="anchor-width: 100%%; anchor-height: 100%%;
                    ...
                    <div class="decorated-container" data-hyui-title="Centrifuge"
    ...
    private static void transferItem(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (s != null && s.page() != null) {
                ...
                updateSlotIcon(page, blockIc, targetSlot, "centrifuge-slot-" + targetSlot);
                try {
    ...
    private static void takeFromBlock(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (ps.page() != null) {
                ...
                updateSlotIcon(page, blockIc, blockSlot, "centrifuge-slot-" + blockSlot);
    ...
    private static void transferOutputToPlayer(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            ItemContainer blockIc = state.getItemContainer();
            if (blockIc == null) return;
            ItemStack output = blockIc.getItemStack((short) 1);
            ...
            blockIc.setItemStackForSlot((short) 1, updated);
            state.uiDirty = true;
    ...
    private static CentrifugeState lookupState(Store<EntityStore> store, Vector3i pos) {
        Object node = CrystallineFluxNetwork.getAt(pos);
        if (node instanceof CentrifugeState s) return s;
        return null;
    }
    ...
    private static final String STYLE = """
            <style>
                .title-label { font-weight: bold; color: #b2dfdb; font-size: 18; padding-top: 8; padding-bottom: 6; }
