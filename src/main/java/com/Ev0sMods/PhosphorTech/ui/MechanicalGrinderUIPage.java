...
/** HyUI page for the Mechanical Grinder block. */
@SuppressWarnings({"removal", "unchecked"})
public final class MechanicalGrinderUIPage {

    private MechanicalGrinderUIPage() {}
    ...
    public static void tickRefresh(MechanicalGrinderState state, Store<?> store, Vector3i pos) {
        SESSIONS.forEach((playerRef, session) -> {
            if (!posKey(session.blockPos()).equals(posKey(pos))) return;
            HyUIPage page = session.page();
            if (page == null) return;
    ...
    private static void renderPage(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            MechanicalGrinderState state = lookupState(pos);
            ...
            builder.addEventListener("mg-close-btn", CustomUIEventBindingType.Activating,
                    (ign, ctx) -> {
            ...
            HytaleLogger.getLogger().atWarning().log("[MechanicalGrinderUI] renderPage failed: " + t);
        }
    }

    private static void partialRefresh(HyUIPage page, MechanicalGrinderState state) {
        if (page == null || state == null) return;

        // Joule bar
        int jFill = (int)(288.0 * state.joulesStored / MechanicalGrinderState.J_CAPACITY);
        page.getById("mg-j-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(jFill));
        page.getById("mg-j-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(String.format("%.1f / %.0f J",
                        state.joulesStored, MechanicalGrinderState.J_CAPACITY)));

        // Speed (only relevant while processing)
        boolean stalled = state.currentSpeed < MechanicalGrinderState.MIN_SPEED;
        ...
        page.getById("mg-speed-val", LabelBuilder.class)
        ...
        page.getById("mg-prog-fill", PanelBuilder.class).ifPresent(b -> b.withContentWidth(progFill));
        String progText = buildProgText(state.processing, state.processTimer, state.ticksNeeded);
        page.getById("mg-prog-text", LabelBuilder.class).ifPresent(b -> b.withText(progText));
        ...
        page.getById("mg-status-val", LabelBuilder.class)
                .ifPresent(b -> b.withText(statusText).withStyle(new HyUIStyle().setTextColor(statusColor)));

        updateSlotIcon(page, state.getItemContainer(), (short) 0, "mg-slot-0");
        updateSlotIcon(page, state.getItemContainer(), (short) 1, "mg-slot-1");
        updateSlotIcon(page, state.getItemContainer(), (short) 2, "mg-slot-2");
    }
    ...
    private static String buildHtml(MechanicalGrinderState state, Inventory inv, List<SlotInfo> slotsOut) {
        boolean processing = state != null && state.processing;
        boolean stalled    = state != null && state.currentSpeed < MechanicalGrinderState.MIN_SPEED;
        String statusColor = processing ? (stalled ? "#ef9a9a" : "#a5d6a7") : "#ef9a9a";
        String statusText  = processing ? (stalled ? "Stalled" : "Processing...") : "Idle";
        String speedColor  = stalled ? "#ef9a9a" : "#a5d6a7";
        // Speed line only shown while processing
        ...
        int jFill = state != null
                ? (int)(288.0 * state.joulesStored / MechanicalGrinderState.J_CAPACITY) : 0;
        String jText = state != null
                ? String.format("%.1f / %.0f J", state.joulesStored, MechanicalGrinderState.J_CAPACITY)
                : "0.0 / 20 J";
        ...
        String slot2Id = null; int slot2Qty = 0;
        if (state != null) {
            ...
            if (ic != null) {
                ...
                ItemStack s2 = ic.getItemStack((short) 2);
                if (s2 != null && !s2.isEmpty()) { slot2Id = s2.getItemId(); slot2Qty = s2.getQuantity(); }
                slotsOut.add(new SlotInfo("mg-slot-0-btn", ic, (short) 0));
                slotsOut.add(new SlotInfo("mg-slot-1-btn", ic, (short) 1));
                slotsOut.add(new SlotInfo("mg-slot-2-btn", ic, (short) 2));
            }
        }
        ...
        String leftPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 330; padding-top: 8; padding-bottom: 8;
                    ...
                    <p class="title-label">Mechanical Crusher</p>
                    <div class="separator"></div>
                    <p class="section-label">Status</p>
                    <p id="mg-status-val" class="info-label" style="color: %s;">%s</p>
                    <p id="mg-speed-val" class="info-label" style="color: %s;">%s</p>
                    <div class="separator"></div>
                    ...
                    <div style="layout-mode: Left; anchor-width: 288; anchor-height: 18;
                        ...
                        <div id="mg-j-fill" style="anchor-width: %d; anchor-height: 18;
                             background-color: #ffa726; border-radius: 9;"></div>
                    </div>
                    <p id="mg-j-val" class="info-label">%s</p>
                    <div class="separator"></div>
                    <p class="section-label">Progress</p>
                    <p id="mg-prog-text" class="info-label" style="margin-top: 4; margin-bottom: 4;">%s</p>
                    ...
                    <div style="layout-mode: Top; horizontal-align: center; padding-top: 8;">
                        <button id="mg-close-btn" class="secondary-button"
                                style="anchor-width: 120; anchor-height: 30; font-size: 13; color: #e57373;">
                            Close</button>
                    </div>
                </div>
                """, statusColor, statusText, speedColor, speedText, jFill, jText, progText);
        ...
        String rightPanel = String.format("""
                <div style="layout-mode: Top; anchor-width: 210; padding-top: 8; padding-bottom: 8;
                    ...
                    %s
                    <p class="hint-label" style="horizontal-align: center;">[Bonus]</p>
                    %s
                </div>
                """, buildSlotHtml(slot0Id, slot0Qty, "Input",  "mg-slot-0"),
                     buildOutputSlotHtml(slot1Id, slot1Qty, "Output", "mg-slot-1"),
                     buildOutputSlotHtml(slot2Id, slot2Qty, "Bonus Output", "mg-slot-2"));
        ...
        return STYLE + String.format("""
                <div style="anchor-width: 100%%; anchor-height: 100%%;
                    ...
                    <div class="decorated-container" data-hyui-title="Mechanical Grinder"
    ...
    private static void transferItem(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (s != null && s.page() != null) {
                ...
                updateSlotIcon(page, blockIc, targetSlot, "mg-slot-" + targetSlot);
                try {
    ...
    private static void takeFromBlock(PlayerRef playerRef, Ref<EntityStore> entityRef,
        ...
        try {
            ...
            if (ps.page() != null) {
                ...
                updateSlotIcon(page, blockIc, blockSlot, "mg-slot-" + blockSlot);
    ...
    private static MechanicalGrinderState lookupState(Vector3i pos) {
        MechanicalGrinderState s = MechanicalGrinderState.REGISTRY.get(posKey(pos));
        if (s != null) return s;
        Object node = GearNetwork.getAt(pos);
        if (node instanceof MechanicalGrinderState gs) return gs;
        return null;
    }
